...
export async function POST(request: Request) {
  try {
    ...
    let query = supabase.from('master_students').select('id, semester, agreed_semester_fee');
    ...
    const ledgerEntries = students.map((student) => ({
      ...
      fee_type: 'Semester',
      total_required: student.agreed_semester_fee, 
      amount_paid: 0.00,
    }));
    ...
    return NextResponse.json({ error: 'Failed to generate semester fees' }, { status: 500 });
  }
}